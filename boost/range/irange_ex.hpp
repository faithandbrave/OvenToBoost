...
//
#ifndef BOOST_RANGE_IRANGE_HPP_INCLUDED // same boost/range/irange.hpp
...
namespace boost { namespace range_detail {
    ...
    template <class>
    struct is_step_keyword : boost::mpl::false_ {};

    template <class Arg>
    struct is_step_keyword<boost::parameter::aux::tagged_argument<step_tag, Arg> >
        : boost::mpl::true_ {};

}} // namespace boost::range_detail

namespace {
    ::boost::parameter::keyword<::boost::range_detail::step_tag> const& _step
       = ::boost::parameter::keyword<::boost::range_detail::step_tag>::instance;
}
...
namespace boost
{
    ...
    template<typename Integer, typename StepArg>
    strided_integer_range<Integer>
        irange_impl(Integer first, Integer last, const StepArg& step_arg, boost::mpl::true_)
    {
        return irange_impl(first, last, step_arg[_step], boost::mpl::false_());
    }

    template<typename Integer, typename StepSize>
    strided_integer_range<Integer>
        irange_impl(Integer first, Integer last, const StepSize& step_size, boost::mpl::false_)
    {
        BOOST_ASSERT( step_size != 0 );
    ...
    template<typename Integer, typename StepSize>
    strided_integer_range<Integer>
        irange(Integer first, Integer last, const StepSize& step_size)
    {
        return irange_impl(first, last, step_size,
                           range_detail::is_step_keyword<StepSize>());
    }
